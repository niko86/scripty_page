...
# TODO: Doesnt update table when values are changed
# TODO: Improve layout
# TODO: Add more widgets for customization
# TODO: Filtering table data should filter plot data and vice versa
...
def uploaded_ags(uploaded_data) -> dict:
    ...
    if uploaded_data:
        ...
        tables = {key: AGS4.convert_to_numeric(value.drop("HEADING", axis=1)) for key, value in tables.items()}
        return tables
    return {}
...
def show_plot(tables: dict[str, pd.DataFrame], table_name: str, x_col: str, y_col: str, group_col: str, marker_size: int):
    ...
    return pn.pane.Bokeh(p, width_policy="fit")



load_ags_bind = pn.bind(uploaded_ags, file_upload)
selection_bind = pn.bind(display_ags_data, load_ags_bind, select_box)
plot_bind = pn.bind(show_plot, load_ags_bind, select_box, x_select, y_select, group_select, marker_size_slider)
...
layout = pn.Column(
    pn.Row(
        pn.Column(
            ...
            pn.Row(y_select),
            pn.Row(group_select),
            pn.Row(marker_size_slider),
        ),
        pn.Column(
            pn.Row(plot_bind),
        ),
    ),
    pn.Row(selection_bind), width_policy="max"
)
